//! `RoomConsistency` at the wire boundary: an empty message, since the modal
//! Room is read straight off currently-placed Sessions — no field to parse
...
fn room_consistency(weight: f64, kinds: Vec<String>) -> pb::ConstraintConfig {
    pb::ConstraintConfig {
        weight,
        applies_to_kinds: kinds,
        ..enabled(
            "c-consistency",
            pb::constraint_config::Params::RoomConsistency(pb::RoomConsistency {}),
        )
    }
}
...
fn kinds_and_weight_reach_the_problem() {
    ...
    input
        .constraints
        .push(room_consistency(4.0, vec!["lecture".into()]));

    let problem = convert(&input, &scope(&["o1"])).expect("valid input");
    assert_eq!(problem.room_consistency_weight, 4.0);
    assert_eq!(problem.constraints.room_consistency[0].kinds, vec!["lecture".to_string()]);
}
...
fn a_negative_weight_is_refused() {
    let mut input = base_input();
    input.offerings = vec![offering("o1", 0)];
    input.constraints.push(room_consistency(-1.0, vec![]));
